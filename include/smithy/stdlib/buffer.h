...
// If the buffer is an alias of something else (i.e. not owned)
static inline bool sm_buffer_is_alias(const sm_buffer buf) {
  return buf.capacity == SIZE_MAX;
}

// WARNING: Unconditional free, even if it's an alias buffer!
...
// NULL, in which case the buffer will be allocated to hold all the data. If the
// buffer is empty, @pos must be NULL. Mimics std::vector::insert.
...
// std::fill. @last may be NULL, in which case it defaults to sm_buffer_end(buf)
